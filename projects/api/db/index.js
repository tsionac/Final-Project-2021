...
const DB_URI = `mongodb+srv://user:souriRE10@clusterfinal.hgsmz.mongodb.net/test?retryWrites=true&w=majority`;
    ...
    mongoose.connect(DB_URI, { useNewUrlParser: true, useUnifiedTopology: true })
        .then((res, err) => {
            ...
            Manager.findOne({ userID }, function(err, result) {
                if (err) {
                    console.log(`FATAL 1 : Admin was not set!`, e);
                    reject(e);
                }
                if (!result) {
                    newManager.save().then((adminDoc) => {
                        ...
                        console.log(`FATAL 2 : Admin was not set!`, e);
                        reject(e);
                    })
                } else {
